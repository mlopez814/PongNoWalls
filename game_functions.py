...
def check_keydown_events(event, stats, sb, paddles, cp, ball):
    if event.key == pygame.K_UP:
        ...
        if sb.check_winner():
            reset_game(stats, sb, paddles, cp, ball)
...
def check_events(stats, sb, paddles, cp, ball):
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            sys.exit()

        elif event.type == pygame.KEYDOWN:
            check_keydown_events(event, stats, sb, paddles, cp, ball)

        elif event.type == pygame.KEYUP:
            check_keyup_events(event, paddles)


def check_ball_edges(screen, stats, sb, paddles, cp, ball):
    screen_rect = screen.get_rect()
    if ball.check_edges():
        pygame.mixer.Sound('sounds/goal.wav').play()
...
def update_ball(ai_settings, screen, stats, sb, paddles, cp, ball):
    ...
    check_ball_edges(screen, stats, sb, paddles, cp, ball)
    ball.update()
...
def reset_serve(paddles, cp, ball):
    ball.reset()
    paddles.reset()
    cp.reset()


def reset_game(stats, sb, paddles, cp, ball):
    reset_serve(paddles, cp, ball)
    stats.reset_stats()
    sb.computer_score()
    sb.player_score()
